  ...
  loadStudents(): void {
    this.studentService.getStudents().subscribe((data: Student[]) => {
      if (data && data.length > 0 && data[0].id !== undefined) {
        ...
        // Handle the case where students are not loaded properly
        console.error('Failed to load students or no students available');
      }
    });
  }
  ...
  loadCourses(): void {
    this.courseService.getCourses().subscribe((data: Course[]) => {
      if (data && data.length > 0 && data[0].id !== undefined) {
        ...
        // Handle the case where courses are not loaded properly
        console.error('Failed to load courses or no courses available');
   }
});
}
